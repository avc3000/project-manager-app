...
export const resolvers = {
  ...
  Mutation: {
    ...
    createTask: async (_, { title, projectId }) => {
      ...
      if (!projectFound) throw new Error(`Project ${projectId} not found`);
    ...
    updateProject: async (_, args) => {
      ...
      if (!updatedProject) throw new Error(`Project ${args.projectId} not found`);

      return updatedProject;
    },
    updateTask: async (_, args) => {
      const updatedTask = await Task.findByIdAndUpdate(args._id, args, { new: true });
      if (!updatedTask) throw new Error(`Task ${args.taskId} not found`);

      return updatedTask;
    },
    deleteProject: async (_, { _id }) => {
      const deletedProject = await Project.findByIdAndDelete(_id);
      if (!deletedProject) throw new Error(`Project ${_id} not found`);
    ...
    deleteTask: async (_, { _id }) => {
      ...
      if (!deletedTask) throw new Error(`Task ${_id} not found`);
